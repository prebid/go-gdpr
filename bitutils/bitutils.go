...
func ParseByte4(data []byte, bitStartIndex uint) (byte, error) {
	...
	if uint(len(data)) < (startByte+2) && bitStartOffset > 3 {
		return 0, fmt.Errorf("ParseByte4 expected 4 bitst to start at bit %d, but the consent string was only %d bytes long", bitStartIndex, len(data))
	}

	leftBits := (data[startByte] & (0xf0 >> bitStartOffset)) << (bitStartOffset - 4)
	bitsConsumed := 7 - bitStartOffset
